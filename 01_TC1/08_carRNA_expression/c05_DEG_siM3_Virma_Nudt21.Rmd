...
work_dir <- "~/Data/01_TC1/02_carRNA_expression/01_p14_siM3Nu/"
require("knitr")
...
```{r 设置工作路径}
#设置工作路径
dir='/disk/user_08/Data/TC1-planB/52_m6Aseq_siM3_siNudt21_siVirma_2nd/03_hisat2_mapping/03_bam_merge/'

sample_name <- paste(rep(c("siNC_caRNA_input","siM3_caRNA_input","siNudt21_caRNA_input"),each=2),
                     rep(c("rep1","rep2"),times=3),
                      sep = "_")

fls <- file.path(dir,paste0("P14_",sample_name,'.bam'))
file.exists(fls)

sample_name <- paste(rep(c("siNC","siMettl3","siNudt21"),each=2),
                     rep(c("1","2"),times=3),
                      sep = "_")
```
...
cd ~/Data/01_TC1/02_carRNA_expression/01_p14_siM3Nu/01_raw_fc

file_dir=/disk/user_08/Data/TC1-planB/52_m6Aseq_siM3_siNudt21_siVirma_2nd/03_hisat2_mapping/03_bam_merge/
file_pre=("siNC" "siM3" "siNudt21")
...
for j in {rep1,rep2}
do
  file="$file ${file_dir}P14_${i}_caRNA_input_${j}.bam"
done
done
...
set_matrix_DEG <- fc_exon_count_rmdup
sample_name1 <- sample_name
meta_data <- data.frame(rep = rep(c("1","2"),times=3),
                        stage = rep(c("siNC","siMettl3","siNudt21"),each=2),
...
res_siNC_siMettl3_filter_1 <- tt_DEG_filter(res_siNC_siMettl3,"siNC","siMettl3",lfcThreshold = 1,save_dir = "02_DEG_result")
res_siNC_siNudt21_filter_1 <- tt_DEG_filter(res_siNC_siNudt21,"siNC","siNudt21",lfcThreshold = 1,save_dir = "02_DEG_result")

res_siMettl3_siNudt21_filter_1 <- tt_DEG_filter(res_siMettl3_siNudt21,"siMettl3","siNudt21",lfcThreshold = 1,save_dir = "02_DEG_result")
...
tt_barplot <- function(gene_name){
  temp <- data.frame(group=factor(stages,levels=c("siNC","siMettl3","siNudt21")),
...
for (i in marker_group){
  heatmap <- Heatmap(matrix = t(scale(t(tpm_matrix[rownames(tpm_matrix) %in% get(paste0("marker_",i)),]))),name="z-score",
...
pdf(paste0("08_marker/02_heatmap_cluster_col_",i,".pdf"))
heatmap <- draw(heatmap)
dev.off()

}

heatmap <- Heatmap(matrix = t(scale(t(tpm_matrix[marker_all,]))),name="z-score",
                                show_column_names = TRUE,show_row_names=T,
...
pdf(paste0("08_marker/heatmap_cluster_col_all.pdf"),height = 25)
heatmap <- draw(heatmap)
dev.off()

dir.create("08_marker/marker_all_km")
...
pdf(paste0("08_marker/marker_all_km/heatmap_cluster_col_marker_km",i,".pdf"),height = 25)
heatmap <- draw(heatmap)
dev.off()
}
...
# 基因表达稳定性分析
geneExpr <- t(tpm_matrix)
geneExpr <- log2(geneExpr + 1)
geneExpr <- na.omit(geneExpr)
geneExpr <- as.data.frame(t(scale(t(geneExpr))))

# 构建共表达网络
powers <- c(c(1:10), seq(from = 12, to = 20, by = 2))
sft <- pickSoftThreshold(geneExpr, powerVector = powers, verbose = 5)
power <- sft$powerEstimate

# 构建网络
networkType <- "signed"
cor <- cor(geneExpr, use = "p")
cor = bicor(cor)
network <- blockwiseModules(cor, power = power, TOMType = networkType, 
                            mergeCutHeight = 0.25, 
                            minModuleSize = 30, 
                            reassignThreshold = 0, 
...
# 模块注释
moduleLabels <- getModuleColorLabels(network)
moduleColors <- labels2colors(moduleLabels)
table(moduleColors)

# 相关性分析
MEs <- moduleEigengenes(geneExpr, colors = moduleColors)$eigengenes
moduleTraitCor <- cor(MEs, trait, use = "p")
moduleTraitPvalue <- corPvalueStudent(moduleTraitCor, nSamples)
moduleTraitSignificance <- moduleTraitPvalue < 0.05
moduleTraitCor <- round(moduleTraitCor, 2)
moduleTraitPvalue <- formatPValue(moduleTraitPvalue, digits = 2)

```

```{r siNC cluster}
dir.create("09_diff_siNC")

diff_geneset_filter1 <- c(rownames(res_siNC_siMettl3_filter_1),
                              rownames(res_siNC_siNudt21_filter_1),
rownames(res_siMettl3_siNudt21_filter_1))

diff_geneset_filter1 <- diff_geneset_filter1[!duplicated(diff_geneset_filter1)]
tpm_diff_siNC <- tpm_matrix[diff_geneset_filter1,]

dir.create("09_diff_siNC/km")
for (i in 1:20){
  heatmap_diff <- Heatmap(matrix = t(scale(t(tpm_diff_siNC))),name="z-score",
...
pdf(paste0("09_diff_siNC/km/heatmap_cluster_col_diff_km",i,".pdf"))
...
pdf(paste0("06_diff_siNC/km_10/heatmap.pdf"))
heatmap_diff <- draw(heatmap_diff)
dev.off()
...
pdf("06_diff_siNC/km_4/heatmap_km4_order.pdf")
heatmap_IR_diff_order <- draw(heatmap_IR_diff_order)
dev.off()


write.table(tpm_newOrder1,"06_diff_siNC/km_4/tpm_newOrder1.txt",quote = F)
write.table(tpm_newOrder2,"06_diff_siNC/km_4/tpm_newOrder2.txt",quote = F)
write.table(tpm_newOrder3,"06_diff_siNC/km_4/tpm_newOrder3.txt",quote = F)
write.table(tpm_newOrder4,"06_diff_siNC/km_4/tpm_newOrder4.txt",quote = F)

write.table(rownames(tpm_newOrder1),"06_diff_siNC/km_4/newOrder1_genes.txt",quote = F,row.names = F,col.names = F)
write.table(rownames(tpm_newOrder2),"06_diff_siNC/km_4/newOrder2_genes.txt",quote = F,row.names = F,col.names = F)
write.table(rownames(tpm_newOrder3),"06_diff_siNC/km_4/newOrder3_genes.txt",quote = F,row.names = F,col.names = F)
write.table(rownames(tpm_newOrder4),"06_diff_siNC/km_4/newOrder4_genes.txt",quote = F,row.names = F,col.names = F)



dir.create("06_diff_siNC/km_4/01_GO")
keytypes(org.Mm.eg.db)


for (i in 1:4){
   assign(paste0("Go_",i),transGO(i,"06_diff_siNC/km_4/01_GO/"))
}
...
```{r siNC cluster M3IN}
dir.create("06_diff_siNC_M3IN")

diff_geneset_filter1 <- c(rownames(res_siNC_p0_filter_1),
...
diff_geneset_filter1 <- diff_geneset_filter1[!duplicated(diff_geneset_filter1)]
tpm_diff_siNC_M3IN <- tpm_matrix_mean[diff_geneset_siNC_filter1,c(1,2,5:9)]

set.seed(1000)

pdf(paste0("06_diff_siNC_M3IN/heatmap_cluster_col_diff_km.pdf"))
for (i in 1:20){
  heatmap_diff <- Heatmap(matrix = na.omit(t(scale(t(tpm_diff_siNC_M3IN)))),name="z-score",
                                show_column_names = TRUE,show_row_names=FALSE,
                                cluster_rows = TRUE,cluster_columns = T,
                                width = unit(4,'inches'),
                        row_km = i,
...
pdf(paste0("06_diff_siNC/km_4/heatmap_km4.pdf"))
