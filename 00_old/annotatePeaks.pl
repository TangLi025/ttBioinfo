#!/usr/bin/env perl
use warnings;
use lib '/disk/user_09/anaconda3/envs/m6A/share/homer/.//bin';
my $homeDir = '/disk/user_09/anaconda3/envs/m6A/share/homer/./';
...
if ($peakFile eq 'tss' || $peakFile eq 'tts' || $peakFile eq 'rna' || -e $specialFile) {
	...
	if ($gtfFile ne '') {
		`parseGTF.pl "$gtfFile" $peakFile $gffFlag $gidFlag -feature exon 3UTR 5UTR > "$gtfTSSFile"`;
