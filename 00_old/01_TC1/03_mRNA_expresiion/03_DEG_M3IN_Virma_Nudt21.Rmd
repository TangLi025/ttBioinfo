...
# ~/Data/01_TC1/13_TC1_P6_siNUDT21/05_DEG_Nudt21_3/05_DEG_Nudt21_3.Rproj
work_dir <- "~/Data/01_TC1/13_TC1_P6_siNUDT21/05_DEG_Nudt21_3"
require("knitr")
...
dir3 <- "/disk/user_08/Data/TC1-planB/11_mRNA/05_bam_hisat2/01_bam_sorted/11_bam_merge/"
sample_name3 <- paste(rep(c("p0","p5","p10","rp2"),each=2),
                      rep(c("input"),each=8),
...
```{bash}
#### 由于mRNA p0p5p10的bam文件有问题 所以在bash上跑featureCounts
cd /disk/user_09/Data/01_TC1/13_TC1_P6_siNUDT21/05_DEG_Nudt21_3
conda activate subread
featureCounts -a /disk/user_09/reference/annotation/mm39/Mus_musculus.GRCm39.108.pc.gtf \
  -o express_matrix_featureCounts_Mus_musculus.GRCm39.108.pc.txt \
  -F GTF -t exon -g 'gene_id' --extraAttributes 'gene_name','gene_biotype' \
...
1> express_matrix_featureCounts_Mus_musculus.GRCm39.108.pc.log 2>&1
```

```{r featurecount_result}
fc_exon <- as.data.frame(fread("express_matrix_featureCounts_Mus_musculus.GRCm39.108.pc.txt"))
rownames(fc_exon) <- fc_exon$Geneid

fc_ERCC <- fc_exon[grep("ERCC",rownames(fc_exon)),]
fc_exon <- fc_exon[grep("ENS",rownames(fc_exon)),]

fc_exon <- na.omit(fc_exon)

fc_exon_count <- fc_exon[,9:32]
colnames(fc_exon_count) <- c(sample_name1,sample_name2,sample_name3)

fc_exon_annotation <- fc_exon[,6:8]

# 
tt_wt(fc_ERCC,file.path("ERCC_raw.txt"),col.names = T,row.names = T) 
...
fc_exon_count_pc <- fc_exon_count[rownames(fc_exon_annotation)[fc_exon_annotation$gene_biotype=="protein_coding"],]
fc_exon_annotation_pc <- fc_exon_annotation[rownames(fc_exon_count_pc),]

table(duplicated(fc_exon_annotation_pc$gene_name))
#View(cbind(fc_exon_annotation_pc[fc_exon_annotation_pc$gene_name %in% fc_exon_annotation_pc[duplicated(fc_exon_annotation_pc$gene_name),"gene_name"],],
#           fc_exon_count_pc[fc_exon_annotation_pc$gene_name %in% fc_exon_annotation_pc[duplicated(fc_exon_annotation_pc$gene_name),"gene_name"],]))

fc_exon_count_pc <- fc_exon_count_pc[!duplicated(fc_exon_annotation_pc$gene_name),]
fc_exon_annotation_pc <- fc_exon_annotation_pc[rownames(fc_exon_count_pc),]

rownames(fc_exon_count_pc) <- fc_exon_annotation_pc$gene_name
rownames(fc_exon_annotation_pc) <- fc_exon_annotation_pc$gene_name


```
### ERCC
```{r ERCC linear}
fc_ERCC_count <- fc_ERCC[,9:32]
colSums(fc_ERCC_count)
filter <- rowSums(fc_ERCC_count>=5)>=2
table(filter)
fc_ERCC_count
...
set_matrix_DEG <- fc_exon_count_pc

XEN_matrix <- read.csv("/disk/user_09/Data/01_TC1/12_TC1_P16_si/XEN_TSC_Merge_count.csv",row.names = 1)

gene_id_name <- read.table("~/reference/annotation/gene_id_name_final_mm.txt",header = T)

venn(list(gene_id_name$mm9,rownames(XEN_matrix)))
venn(list(gene_id_name$mm10,rownames(XEN_matrix)))
venn(list(gene_id_name$mm39,rownames(XEN_matrix)))

venn(list(gene_id_name$mm39,rownames(set_matrix_DEG)))

XEN_matrix <- XEN_matrix[rownames(XEN_matrix) %in% gene_id_name$mm10,]

gene_id_name_use <- gene_id_name[gene_id_name$mm10 %in% rownames(XEN_matrix),c(3,4)]
table(is.na(gene_id_name_use$mm39))
gene_id_name_use <- gene_id_name_use[!is.na(gene_id_name_use$mm39),]
table(duplicated(gene_id_name_use$mm39))
gene_id_name_use <- gene_id_name_use[!duplicated(gene_id_name_use$mm39),]
table(duplicated(gene_id_name_use$mm10))
gene_id_name_use <- gene_id_name_use[!duplicated(gene_id_name_use$mm10),]

XEN_matrix <- XEN_matrix[gene_id_name_use$mm10,]

table(is.na(XEN_matrix))
rownames(XEN_matrix) <- gene_id_name_use$mm39


venn(list(rownames(set_matrix_DEG),rownames(XEN_matrix)))

venn(list(rownames(set_matrix_DEG[rowSums(set_matrix_DEG>=10)>=2,]),rownames(XEN_matrix)))

set_matrix_DEG <- set_matrix_DEG[intersect(rownames(set_matrix_DEG),rownames(XEN_matrix)),]
XEN_matrix_DEG <- XEN_matrix[rownames(set_matrix_DEG),]
set_matrix_DEG <- cbind(set_matrix_DEG,XEN_matrix_DEG)

sample_name <- colnames(set_matrix_DEG)
meta_data <- data.frame(rep = c(rep(c("rep1","rep2"),times=12),rep("rep1",6)),
                        stage = c(rep(c("p6_siNC","p6_siNudt21_1","p6_siNudt21_3",
                                             "Ctrl","M3IN","siNC","siM3","siVirma",
                                             "p0_total","p5_total","p10_total","rp2_total"),each=2),
...
# # 
express_sva <- ComBat_seq(counts=as.matrix(set_matrix_DEG),batch=batch)
...
```{r betweenLaneNormalization rm XEN}
set_matrix_DEG <- fc_exon_count_pc

sample_name <- colnames(set_matrix_DEG)
meta_data <- data.frame(rep = c(rep(c("rep1","rep2"),times=12)),
                        stage = c(rep(c("p6_siNC","p6_siNudt21_1","p6_siNudt21_3",
                                             "Ctrl","M3IN","siNC","siM3","siVirma",
                                             "p0_total","p5_total","p10_total","rp2_total"),each=2)),
                        row.names = sample_name,
...
## 批次校正
batch <- c(rep("p6si",6),rep("p16si",10),rep("mRNA",8))
# # 
express_sva <- ComBat_seq(counts=as.matrix(set_matrix_DEG),batch=batch)
...
express_matrix_rm <- express_matrix[,c(3,4,9,10,15,16,17:18,21:24)]
colnames(express_matrix_rm) <- c("siNudt21_rep1","siNudt21_rep2","M3IN_rep1","M3IN_rep2","siVirma_rep1","siVirma_rep2","p0_rep1","p0_rep2","p10_rep1","p10_rep2","rp2_rep1","rp2_rep2")

express_matrix_rm <- express_matrix_rm[,c(7:12,1,2,5,6,3,4)]

tt_wt(express_matrix_rm,"express_matrix_rmXEN.txt",row.names=T,col.names = T)

```

```{r embryo data}
embryo_percent <- read.table("~/Data/02_embryo/01_mouse_embryo_scRNA-seq_2014_Ding/fpkm_matrix_mean_filter_mm9_mm39_percent.txt",header = T)
rownames(embryo_percent) <- embryo_percent$mm39
embryo_percent <- embryo_percent[,3:12]
...
tpm_matrix <- do.call(cbind,base::lapply(colnames(express_matrix),FUN = function(x){
  ...
  effLen <- fc_exon_annotation_pc[rownames(express_matrix),"Length"]
...
dds <- DESeqDataSetFromMatrix(countData = express_sva,colData = meta_data,design = ~ stage)
...
pdf("01_pca/01_pca_rlog_ensembl_pc_all_min30.pdf",width = 8,height = 7)
print(p1)
dev.off()
...
### only for PCA
```{r create dds object and plot pca remove NC ,Mettl3in}
...
express_DEG <- express_matrix[,c(3:4,9:10,17:18,21:24,27,29,30)]
colnames(express_DEG) <- sample_name

meta_data <- data.frame(rep = c(rep(c("1","2"),times=2),
                                rep(c("1","2"),times=3),rep("1",1),"1","2"),
                        stage = c(rep(c("siNudt21","M3IN"),each=2),
                                  rep(c("p0","p10","rp2"),each=2),
                                  "TSC","XEN","XEN"),
                        row.names = sample_name,
                        sample_name = sample_name,
                        stringsAsFactors = TRUE)
...
pdf("01_pca/01_pca_vst_ensembl_pc_all_min30_M3IN.pdf",width = 8,height = 7)
print(p1)
dev.off()
```

```{r create dds object and plot pca remove NC ,Mettl3in,rm rp2}
...
express_DEG <- express_matrix[,c(3:4,9:10,17:18,21:22,27,29,30)]
colnames(express_DEG) <- sample_name

meta_data <- data.frame(rep = c(rep(c("1","2"),times=2),
                                rep(c("1","2"),times=2),rep("1",1),"1","2"),
                        stage = c(rep(c("siNudt21","M3IN"),each=2),
                                  rep(c("p0","p10"),each=2),
                                  "TSC","XEN","XEN"),
                        row.names = sample_name,
...
pdf("01_pca/01_pca_vst_ensembl_pc_all_min30_M3IN_rmrp2.pdf",width = 7,height = 6)
print(p1)
dev.off()
```

```{r create dds object and plot pca remove NC ,Mettl3in,rm rp2,marker genes}

meta_data 
colnames(express_matrix)

marker_all <- genes_marker
...
dds <- DESeqDataSetFromMatrix(countData = express_DEG[rownames(express_DEG) %in% marker_all,],colData = meta_data,design = ~ stage)

#pca plot
vst <- vst(dds,blind =FALSE,nsub=100)

pcaData <- plotPCA(vst,intgroup = c("stage"),returnData = TRUE)
pcaData
percentVar <- round(100 * attr(pcaData,"percentVar"))
percentVar
...
pdf("01_pca/01_pca_vst_ensembl_pc_all_min30_M3IN_rmrp2_marker.pdf",width = 7,height = 6)
print(p1)
dev.off()
```
...
express_DEG <- express_matrix[,c(3:4,9:10,14,17:18,21:22,27,29,30)]
colnames(express_DEG) <- sample_name
...
pdf("01_pca/01_pca_vst_ensembl_pc_all_min30_M3IN_siM3_rmrp2_marker.pdf",width = 7,height = 6)
print(p1)
dev.off()
```

```{r create dds object and plot pca remove NC, siVirma}

meta_data 
colnames(express_matrix)

sample_name <- c("M3IN_1","M3IN_2",
                 "siVirma_1","siVirma_2",
                 "p0_1","p0_2",
                 "p10_1","p10_2",
                 "rp2_1","rp2_2",
...
express_DEG <- express_matrix[,c(9:10,15:18,21:24,27,29,30)]
colnames(express_DEG) <- sample_name

meta_data <- data.frame(rep = c(rep(c("1","2"),times=1),
                                rep(c("1","2"),times=4),rep("1",1),"1","2"),
                        stage = c(rep(c("M3IN"),each=2),
...
pdf("01_pca/01_pca_vst_ensembl_pc_all_min30_siVirma.pdf",width = 7,height = 6)
print(p1)
dev.off()
```

```{r create dds object and plot pca remove NC, siVirma,rm rp2}
...
express_DEG <- express_matrix[,c(9:10,15:18,21:22,27,29,30)]
colnames(express_DEG) <- sample_name

meta_data <- data.frame(rep = c(rep(c("1","2"),times=1),
                                rep(c("1","2"),times=3),rep("1",1),"1","2"),
                        stage = c(rep(c("M3IN"),each=2),
                                  rep(c("siVirma","p0","p10"),each=2),
                                  "TSC","XEN","XEN"),
                        row.names = sample_name,
                        sample_name = sample_name,
                        stringsAsFactors = TRUE)
...
pdf("01_pca/01_pca_vst_ensembl_pc_all_min30_siVirma_rm_rp2.pdf",width = 7,height = 6)
print(p1)
dev.off()
```
...
pdf("01_pca/01_pca_vst_ensembl_pc_all_min30_siVirma_rm_rp2_marker.pdf",width = 7,height = 6)
print(p1)
dev.off()
```
...
tpm_DEG <- tpm_matrix[,c(3:4,9:10,15:18,21:24,27,29,30)]
colnames(tpm_DEG) <- sample_name

tpm_DEG_mean <- data.frame(siNudt21=apply(tpm_DEG[,1:2],1,mean),
                           M3IN=apply(tpm_DEG[,3:4],1,mean),
                           siVirma=apply(tpm_DEG[,5:6],1,mean),
                           p0=apply(tpm_DEG[,7:8],1,mean),
                           p10=apply(tpm_DEG[,9:10],1,mean),
                           rp2=apply(tpm_DEG[,11:12],1,mean),
                           TSC=tpm_DEG[,13],
                           XEN=apply(tpm_DEG[,14:15],1,mean),
                           row.names = rownames(tpm_DEG))

express_DEG <- express_matrix[,c(3:4,9:10,15:18,21:24,27,29,30)]
colnames(express_DEG) <- sample_name
...
pdf("01_pca/01_pca_rlog_ensembl_pc_all_min30_rm3.pdf",width = 8,height = 7)
print(p1)
dev.off()
...
pdf("01_pca/01_pca_vst_ensembl_pc_all_min30_rm5.pdf",width = 8,height = 7)
print(p1)
dev.off()
```
