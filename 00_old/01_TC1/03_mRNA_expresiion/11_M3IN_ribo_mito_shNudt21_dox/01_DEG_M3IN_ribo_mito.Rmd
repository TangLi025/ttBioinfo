...
# ~/Data/01_TC1/01_mRNA_expression/11_M3IN_ribo_mito_shNudt21_dox

require("knitr")
opts_knit$set(root.dir = file.path("~/Data/01_TC1/01_mRNA_expression/11_M3IN_ribo_mito_shNudt21_dox/"))

```

```{r read express matrix & rm batch effect & betweenLaneNormalization & expression filter10}
...
## 批次校正
batch <- c(rep("ribo_mito",10),rep("shNudt21",8))
express_sva <- ComBat_seq(counts=as.matrix(express_matrix),batch=batch)


x <- meta_data$stage
set <- newSeqExpressionSet(as.matrix(express_sva),
                           phenoData = data.frame(x, row.names=colnames(express_sva)))
set
...
marker_TE <- read.table("~/Data/01_TC1/00_gene_set/all_20230213_p0p10rp2//TE")$V1
...
tt_barplot <- function(gene_name){
  temp <- data.frame(sample=factor(sample_name,levels = sample_name),
                   tpm=tpm_matrix[gene_name,],
                   group=c(rep("ribo",10),rep("shNudt21",8)))
  p1 <- ggplot(temp, aes(x = sample, y = tpm,color=group)) +
  geom_col(position="dodge",width =0.8,show.legend = F,size=1,fill="white")+ #绘制柱状图
  ...
  theme(plot.title = element_text(size=16,hjust = -0.5))+
    ...
    scale_color_manual(values = c("ribo"="#ecc37f","shNudt21"="#c5c5e0"))
  print(p1)
}
...
pdf(paste0("08_marker/07_heatmap_cluster_col_",i,".pdf"))
...
```{r marker barplot ribo+mito+P0P5Ctrl}

sample_select <- c(1:10,11,17)

tt_barplot <- function(gene_name){
  temp <- data.frame(sample=factor(sample_name[sample_select],levels = sample_name[sample_select]),
                   tpm=tpm_matrix[gene_name,c(sample_select)],
                   group=c(rep("ribo",10),rep("shNudt21",2)))
  p1 <- ggplot(temp, aes(x = sample, y = tpm,color=group)) +
  geom_col(position="dodge",width =0.8,show.legend = F,size=1,fill="white")+ #绘制柱状图
  labs(x=NULL,y=paste0(gene_name," tpm"))+#标题
  theme_prism(palette = "floral",
...
pdf("08_marker/01_TE_TSC_barplot.pdf",width =  5,height = 4)
for (i in marker_TE_TSC[marker_TE_TSC %in% rownames(tpm_matrix)]){
  tt_barplot(i)
}
dev.off()

pdf("08_marker/01_TE_barplot.pdf",width =  5,height = 4)
for (i in marker_TE[marker_TE %in% rownames(tpm_matrix)]){
  tt_barplot(i)
}
dev.off()
...
```{r marker heatmap ribo+mito+P5Ctrl}
group <- c("toti","pluri","PrE_XEN","TE","TE_TSC","APAperturb")

for (i in group){
  temp <- t(scale(t(tpm_matrix[rownames(tpm_matrix) %in% get(paste0("marker_",i)),sample_select])))
  temp <- na.omit(temp)
  heatmap <- Heatmap(matrix = temp,name="z-score",
                                show_column_names = TRUE,show_row_names=T,
                                cluster_rows = TRUE,cluster_columns = F,
...
pdf(paste0("08_marker/08_heatmap_cluster_col_",i,"_ribo_mito_P5Ctrl.pdf"),height = 10)
heatmap <- draw(heatmap)
dev.off()
...
fa.parallel(t(express_matrix), fa="pc", n.iter=100,
...
table(tpm_matrix[,7] >0.5 & tpm_matrix[,7] >0.5 & tpm_matrix[,7]/tpm_matrix[,2]>3)
table(tpm_matrix[,7] >0.5 & tpm_matrix[,7] >0.5 & tpm_matrix[,2]/tpm_matrix[,7]>3)

tpm_matrix[tpm_matrix[,7] >0.5 & tpm_matrix[,2] >0.5 & (tpm_matrix[,2]/tpm_matrix[,7])>3,c(2,7)]

DEG_CX_c1_up <- tpm_matrix[(tpm_matrix[,7] >0.5 | tpm_matrix[,2] >0.5) & log2((tpm_matrix[,7]+0.001)/(tpm_matrix[,2]+0.001))>=1.6,c(2,7)]

DEG_CX_c1_up_gene <- rownames(DEG_CX_c1_up)

DEG_CX_c1_down <- tpm_matrix[(tpm_matrix[,7] >0.5 | tpm_matrix[,2] >0.5) & log2((tpm_matrix[,7]+0.001)/(tpm_matrix[,2]+0.001))<= -1.6,c(2,7)]

DEG_CX_c1_down_gene <- rownames(DEG_CX_c1_down)

temp <- tpm_matrix[rownames(tpm_matrix) %in% c(rownames(DEG_CX_c1_up),rownames(DEG_CX_c1_down)),c(2,7,11,17)]
temp <- t(scale(t(temp)))
...
pdf(paste0("05_DEG_heatmap/01_M3IN_vs_CX_c1.pdf"))
heatmap <- draw(heatmap)
dev.off()

### 
DEG_P0_P5_up <- tpm_matrix[(tpm_matrix[,17] >0.5 | tpm_matrix[,11] >0.5) & log2((tpm_matrix[,17]+0.001)/(tpm_matrix[,11]+0.001))>=1.6,c(11,17)]

DEG_P0_P5_up_gene <- rownames(DEG_P0_P5_up)

DEG_P0_P5_down <- tpm_matrix[(tpm_matrix[,17] >0.5 | tpm_matrix[,11] >0.5) & log2((tpm_matrix[,17]+0.001)/(tpm_matrix[,2]+0.001))<= -1.6,c(11,17)]

DEG_P0_P5_down_gene <- rownames(DEG_P0_P5_down)

temp <- tpm_matrix[rownames(tpm_matrix) %in% c(rownames(DEG_P0_P5_up),rownames(DEG_P0_P5_down)),c(2,7,11,17)]
temp <- t(scale(t(temp)))
...
pdf(paste0("05_DEG_heatmap/01_P0_vs_P5.pdf"))
heatmap <- draw(heatmap)
dev.off()

temp <- tpm_matrix[rownames(tpm_matrix) %in% c(rownames(DEG_P0_P5_up),rownames(DEG_P0_P5_down),rownames(DEG_CX_c1_up),rownames(DEG_CX_c1_down)),c(2,7,11,17)]
temp <- t(scale(t(temp)))
...
pdf(paste0("05_DEG_heatmap/01_P0_P5_or_CX_c1.pdf"))
heatmap <- draw(heatmap)
dev.off()

temp <- tpm_matrix[rownames(tpm_matrix) %in% c(intersect(rownames(DEG_P0_P5_up),rownames(DEG_CX_c1_up)),intersect(rownames(DEG_P0_P5_down),rownames(DEG_CX_c1_down))),c(2,7,11,17)]
temp <- t(scale(t(temp)))
...
pdf(paste0("05_DEG_heatmap/01_P0_P5_and_CX_c1.pdf"))
heatmap <- draw(heatmap)
dev.off()

venn.diagram(x=list(P0_P5_up=DEG_P0_P5_up_gene,
                    CX_c1_up=DEG_CX_c1_up_gene),filename = "04_DEG_result/P0_P5_up_CX_c1_up.png",imagetype = "png",
                          fill=c("#EECE13","#B210FF"),col=c("#EECE13","#B210FF"),width=2000,height = 2000,
               cat.just=list(c(-1,-0.5),c(1,0.5)),disable.logging = T)

venn.diagram(x=list(P0_P5_down=DEG_P0_P5_down_gene,
                    CX_c1_down=DEG_CX_c1_down_gene),filename = "04_DEG_result/P0_P5_down_CX_c1_down.png",imagetype = "png",
                          fill=c("#EECE13","#B210FF"),col=c("#EECE13","#B210FF"),width=2000,height = 2000,
               cat.just=list(c(1,0.5),c(-1,-0.5)),disable.logging = T)
...
```{r DEG analysis CX_c2}

DEG_CX_c2_up <- tpm_matrix[(tpm_matrix[,8] >0.5 | tpm_matrix[,2] >0.5) & log2((tpm_matrix[,8]+0.001)/(tpm_matrix[,2]+0.001))>=1.6,c(2,8)]

DEG_CX_c2_up_gene <- rownames(DEG_CX_c2_up)

DEG_CX_c2_down <- tpm_matrix[(tpm_matrix[,8] >0.5 | tpm_matrix[,2] >0.5) & log2((tpm_matrix[,8]+0.001)/(tpm_matrix[,2]+0.001))<= -1.6,c(2,8)]

DEG_CX_c2_down_gene <- rownames(DEG_CX_c2_down)

temp <- tpm_matrix[rownames(tpm_matrix) %in% c(rownames(DEG_CX_c2_up),rownames(DEG_CX_c2_down)),c(2,8,11,17)]
temp <- t(scale(t(temp)))
...
pdf(paste0("05_DEG_heatmap/01_M3IN_vs_CX_c2.pdf"))
heatmap <- draw(heatmap)
dev.off()

temp <- tpm_matrix[rownames(tpm_matrix) %in% c(rownames(DEG_P0_P5_up),rownames(DEG_P0_P5_down)),c(2,8,11,17)]
temp <- t(scale(t(temp)))
...
pdf(paste0("05_DEG_heatmap/01_P0_vs_P5_c2.pdf"))
heatmap <- draw(heatmap)
dev.off()

temp <- tpm_matrix[rownames(tpm_matrix) %in% c(rownames(DEG_P0_P5_up),rownames(DEG_P0_P5_down),rownames(DEG_CX_c2_up),rownames(DEG_CX_c2_down)),c(2,8,11,17)]
temp <- t(scale(t(temp)))
...
pdf(paste0("05_DEG_heatmap/01_P0_P5_or_CX_c2.pdf"))
heatmap <- draw(heatmap)
dev.off()

temp <- tpm_matrix[rownames(tpm_matrix) %in% c(intersect(rownames(DEG_P0_P5_up),rownames(DEG_CX_c2_up)),intersect(rownames(DEG_P0_P5_down),rownames(DEG_CX_c2_down))),c(2,8,11,17)]
temp <- t(scale(t(temp)))
...
pdf(paste0("05_DEG_heatmap/01_P0_P5_and_CX_c2.pdf"))
heatmap <- draw(heatmap)
dev.off()

venn.diagram(x=list(P0_P5_up=DEG_P0_P5_up_gene,
                    CX_c2_up=DEG_CX_c2_up_gene),filename = "04_DEG_result/P0_P5_up_CX_c2_up.png",imagetype = "png",
                          fill=c("#EECE13","#B210FF"),col=c("#EECE13","#B210FF"),width=2000,height = 2000,
               cat.just=list(c(-1,-0.5),c(1,0.5)),disable.logging = T)

venn.diagram(x=list(P0_P5_down=DEG_P0_P5_down_gene,
                    CX_c2_down=DEG_CX_c2_down_gene),filename = "04_DEG_result/P0_P5_down_CX_c2_down.png",imagetype = "png",
                          fill=c("#EECE13","#B210FF"),col=c("#EECE13","#B210FF"),width=2000,height = 2000,
               cat.just=list(c(1,0.5),c(-1,-0.5)),disable.logging = T)


tt_enrichGO(genelist = intersect(rownames(DEG_P0_P5_up),rownames(DEG_CX_c1_up)),groupname = "P0_P5_up_CX_c1_up",filedir = "07_DEG_GO/")
tt_enrichGO(genelist = intersect(rownames(DEG_P0_P5_down),rownames(DEG_CX_c1_down)),groupname = "P0_P5_down_CX_c1_down",filedir = "07_DEG_GO/")
