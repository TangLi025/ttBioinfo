...
work_dir <- "~/Data/09_PseudoU/04_aging_mRNA/03_hisat2_mapping/06_DEG_new2/"
require("knitr")
...
fc_result_annotation$GeneID <- str_sub(fc_result_annotation$GeneID,1,15)
...
```{R}
sample_name <-  paste(rep(c("IC_shCtrl","IR_shCtrl","IR_shDKC1"),each=2),
                      rep(c("1","2"),times=3),
                      sep = "_")
express_matrix <- fc_result_count[,c(5,6,9:12)]
colnames(express_matrix) <- sample_name
meta_data <- data.frame(rep = rep(c("1","2"),times=3),
                        stage = rep(c("IC_shCtrl","IR_shCtrl","IR_shDKC1"),each=2),
...
#load meta data
sample_name <- colnames(express_matrix_filter10)
meta_data <- data.frame(rep = c(rep(c("1","2"),times=3)),
                        stage = factor(rep(c("IC_shCtrl","IR_shCtrl","IR_shDKC1"),each=2),
                                       levels = c("IC_shCtrl","IR_shCtrl","IR_shDKC1")),
...
sn <- c("IC_shCtrl","IR_shCtrl","IR_shDKC1")
...
res_IR_shDKC1_filter_1 <- tt_DEG_filter(res_IR_shDKC1,"IR_shCtrl","IR_shDKC1","04_DEG_result",lfcThreshold = 1)
res_IC_IR_shCtrl_filter_1 <- tt_DEG_filter(res_IC_IR_shCtrl,"IC_shCtrl","IR_shCtrl","04_DEG_result",lfcThreshold = 1)
