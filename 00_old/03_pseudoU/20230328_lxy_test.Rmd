...
# ~/Data/01_TC1/21_M3IN_ca_12h/03_hisat2_mapping/06_DEG_new2
...
#设置工作路径
dir1='/disk/user_03/ToTL/'
temp <- expand.grid(c("rep1","rep2"),c("DMSO","Nira"),c("shCtrl","shDKC1_1"))
sample_name1 <- paste("Hela",temp$Var3,temp$Var2,temp$Var1,sep = "_")

fls <- c(file.path(dir1,paste0(sample_name1,'.genome.bam')))
file.exists(fls)

fc_result <- featureCounts(fls,annot.ext = "/disk/user_09/reference/annotation/hg38/gencode.v40.basic.annotation.pc.gtf",
...
meta_data <- data.frame(rep = rep(c("1","2"),times=4),
                        stage = paste(rep(c("shCtrl","shDKC1"),each=4),rep(c("DMSO","Nira"),each=2,times=2),sep="_"),
...
dds <- DESeqDataSetFromMatrix(countData = express_matrix_filter10[,],colData = meta_data_,design = ~ stage)
...
pdf("01_allgene_rlog.pdf",width = 8,height = 7)
print(p1)
dev.off()
```
...
sn <- c("shCtrl_DMSO","shCtrl_Nira","shDKC1_DMSO","shDKC1_Nira")

res_shCtrl_DMSO_shCtrl_Nira <- tt_DEG_result(dds,"shCtrl_DMSO","shCtrl_Nira","04_DEG_result",independentFiltering=F)
res_shCtrl_DMSO_shDKC1_DMSO <- tt_DEG_result(dds,"shCtrl_DMSO","shDKC1_DMSO","04_DEG_result",independentFiltering=F)
res_shCtrl_DMSO_shDKC1_Nira <- tt_DEG_result(dds,"shCtrl_DMSO","shDKC1_Nira","04_DEG_result",independentFiltering=F)

res_shCtrl_Nira_shDKC1_DMSO <- tt_DEG_result(dds,"shCtrl_Nira","shDKC1_DMSO","04_DEG_result",independentFiltering=F)
res_shCtrl_Nira_shDKC1_Nira <- tt_DEG_result(dds,"shCtrl_Nira","shDKC1_Nira","04_DEG_result",independentFiltering=F)

res_shDKC1_DMSO_shDKC1_Nira <- tt_DEG_result(dds,"shDKC1_DMSO","shDKC1_Nira","04_DEG_result",independentFiltering=F)

summary(res_shCtrl_DMSO_shCtrl_Nira)#
summary(res_shCtrl_DMSO_shDKC1_DMSO)#
summary(res_shCtrl_DMSO_shDKC1_Nira)

summary(res_shCtrl_Nira_shDKC1_DMSO)
summary(res_shCtrl_Nira_shDKC1_Nira)#

summary(res_shDKC1_DMSO_shDKC1_Nira)

res_p0p10 <- read.table("~/Data/01_TC1/01_mRNA_expression/01_p0p5p10rp2/04_DEG_result/res_p0_p10.txt")

res_sample <- DESeq2::results(dds,contrast=c("stage","shCtrl_DMSO","shCtrl_Nira"),lfcThreshold = 0,alpha = 0.99,independentFiltering = F)
  summary(res_sample)
  
plotMA(res_shCtrl_DMSO_shDKC1_Nira, ylim=c(-2,2))

plot(metadata(res_shCtrl_DMSO_shDKC1_DMSO)$filterNumRej, 
     type="b", ylab="number of rejections",
     xlab="quantiles of filter")
lines(metadata(res_shCtrl_DMSO_shDKC1_DMSO)$lo.fit, col="red")
abline(v=metadata(res_shCtrl_DMSO_shDKC1_DMSO)$filterTheta)

plot(metadata(res_shCtrl_DMSO_shDKC1_Nira)$filterNumRej, 
     type="b", ylab="number of rejections",
     xlab="quantiles of filter")
lines(metadata(res_shCtrl_DMSO_shDKC1_Nira)$lo.fit, col="red")
abline(v=metadata(res_shCtrl_DMSO_shDKC1_Nira)$filterTheta)
...
#filter1
#### 
res_shCtrl_DMSO_shCtrl_Nira_filter_1 <- tt_DEG_filter(res_shCtrl_DMSO_shCtrl_Nira,"shCtrl_DMSO","shCtrl_Nira","04_DEG_result",lfcThreshold = 1)
res_shCtrl_DMSO_shDKC1_DMSO_filter_1 <- tt_DEG_filter(res_shCtrl_DMSO_shCtrl_Nira,"shCtrl_DMSO","shDKC1_DMSO","04_DEG_result",lfcThreshold = 0.58)
res_shDKC1_DMSO_shDKC1_Nira_filter_1 <- tt_DEG_filter(res_shDKC1_DMSO_shDKC1_Nira,"shDKC1_DMSO","shDKC1_Nira","04_DEG_result",lfcThreshold = 1)

filter <- res_shCtrl_DMSO_shDKC1_Nira[abs(res_shCtrl_DMSO_shDKC1_Nira$log2FoldChange) > 1 & res_shCtrl_DMSO_shDKC1_Nira$padj < 0.05,]
  summary(filter)

tt_DEG_heatmap(res_IR_shDKC1_filter_1,"IR_shCtrl","IR_shDKC1",tpm_matrix,meta_data,save_dir = "05_DEG_heatmap/",lfcThreshold = 1)
tt_DEG_heatmap(res_IC_IR_shCtrl_filter_1,"IC_shCtrl","IR_shCtrl",tpm_matrix,meta_data,save_dir = "05_DEG_heatmap/",lfcThreshold = 1)
...
tt_enrichGO(genelist = rownames(res_IC_IR_shCtrl_filter_1)[res_IC_IR_shCtrl_filter_1$log2FoldChange>0],groupname = "IC_IR_shCtrl_filter1_up",OrgDb = org.Hs.eg.db,filedir = "07_DEG_GO/",width = 8)
tt_enrichGO(genelist = rownames(res_IC_IR_shCtrl_filter_1)[res_IC_IR_shCtrl_filter_1$log2FoldChange<0],groupname = "IC_IR_shCtrl_filter1_down",OrgDb = org.Hs.eg.db,filedir = "07_DEG_GO/",width = 8)

tt_enrichGO(genelist = rownames(res_IR_shDKC1_filter_1)[res_IR_shDKC1_filter_1$log2FoldChange>0],groupname = "IR_shDKC1_filter1_up",OrgDb = org.Hs.eg.db,filedir = "07_DEG_GO/",width = 10)
tt_enrichGO(genelist = rownames(res_IR_shDKC1_filter_1)[res_IR_shDKC1_filter_1$log2FoldChange<0],groupname = "IR_shDKC1_filter1_down",OrgDb = org.Hs.eg.db,filedir = "07_DEG_GO/",width = 8)

```

```{r}

pdf("05_DEG_heatmap/filter_1/heatmap_reverse.pdf")
Heatmap(t(scale(t(tpm_matrix[rownames(tpm_matrix) %in% c(intersect(rownames(res_IC_IR_shCtrl_filter_1[res_IC_IR_shCtrl_filter_1$log2FoldChange>1,]),rownames(res_IR_shDKC1_filter_1[res_IR_shDKC1_filter_1$log2FoldChange< -1,])),intersect(rownames(res_IC_IR_shCtrl_filter_1[(res_IC_IR_shCtrl_filter_1$log2FoldChange< -1)==TRUE,]),rownames(res_IR_shDKC1_filter_1[(res_IR_shDKC1_filter_1$log2FoldChange>1)==TRUE,]))),]))),
        col=circlize::colorRamp2(c(-1.5,-0.5,1.2),
...
venn.diagram(x=list(IRup=rownames(res_IC_IR_shCtrl_filter_1[res_IC_IR_shCtrl_filter_1$log2FoldChange>0,]),shDKC1down=rownames(res_IR_shDKC1_filter_1[res_IR_shDKC1_filter_1$log2FoldChange< 0,])),filename = "04_DEG_result/filter_1/IR_up_shDKC1_down.png",imagetype = "png",
             fill=c("#EECE13","#B210FF"),col=c("#EECE13","#B210FF"),width=2000,height = 2000,
               cat.just=list(c(3,0),c(-0.6,0)))

VennDiagram::venn.diagram(x=list(IRup=rownames(res_IC_IR_shCtrl_filter_1[res_IC_IR_shCtrl_filter_1$log2FoldChange<0,]),shDKC1down=rownames(res_IR_shDKC1_filter_1[res_IR_shDKC1_filter_1$log2FoldChange> 0,])),filename = "04_DEG_result/filter_1/IR_down_shDKC1_up.png",imagetype = "png",
                          fill=c("#EECE13","#B210FF"),col=c("#EECE13","#B210FF"),width=2000,height = 2000,
               cat.just=list(c(3,0),c(-0.6,0)))
...
dir.create("05_DEG_heatmap/filter_058")
lfcThreshold <- 0.58

res_IR_shDKC1_filter_058 <- tt_DEG_filter(res_IR_shDKC1,"IR_shCtrl","IR_shDKC1","04_DEG_result",lfcThreshold =lfcThreshold)
res_IC_IR_shCtrl_filter_058 <- tt_DEG_filter(res_IC_IR_shCtrl,"IC_shCtrl","IR_shCtrl","04_DEG_result",lfcThreshold =lfcThreshold)
```

```{r shDKC1 reverse}
pdf("05_DEG_heatmap/filter_058/heatmap_reverse.pdf")
Heatmap(t(scale(t(tpm_matrix[rownames(tpm_matrix) %in% c(intersect(rownames(res_IC_IR_shCtrl_filter_058[res_IC_IR_shCtrl_filter_058$log2FoldChange>0,]),rownames(res_IR_shDKC1_filter_058[res_IR_shDKC1_filter_058$log2FoldChange< 0,])),intersect(rownames(res_IC_IR_shCtrl_filter_058[res_IC_IR_shCtrl_filter_058$log2FoldChange< 0,]),rownames(res_IR_shDKC1_filter_058[res_IR_shDKC1_filter_058$log2FoldChange>0,]))),]))),
        col=circlize::colorRamp2(c(-1.5,-0.5,1.2),
                               colorRampPalette(c("#4875aa", "white", "#d7604c"))(3)),
        cluster_columns = F,
        show_row_names = F)
...
test <- tpm_matrix[rownames(tpm_matrix) %in% c(intersect(rownames(res_IC_IR_shCtrl_filter_058[res_IC_IR_shCtrl_filter_058$log2FoldChange>0,]),rownames(res_IR_shDKC1_filter_058[res_IR_shDKC1_filter_058$log2FoldChange< 0,])),intersect(rownames(res_IC_IR_shCtrl_filter_058[res_IC_IR_shCtrl_filter_058$log2FoldChange< 0,]),rownames(res_IR_shDKC1_filter_058[res_IR_shDKC1_filter_058$log2FoldChange>0,]))),]

tt_wt(intersect(rownames(res_IC_IR_shCtrl_filter_058[res_IC_IR_shCtrl_filter_058$log2FoldChange>0,]),rownames(res_IR_shDKC1_filter_058[res_IR_shDKC1_filter_058$log2FoldChange< 0,])),"04_DEG_result/filter_0.58/IR_up_shDKC1_down.txt")
tt_wt(intersect(rownames(res_IC_IR_shCtrl_filter_058[res_IC_IR_shCtrl_filter_058$log2FoldChange<0,]),rownames(res_IR_shDKC1_filter_058[res_IR_shDKC1_filter_058$log2FoldChange> 0,])),"04_DEG_result/filter_0.58/IR_down_shDKC1_up.txt")

venn.diagram(x=list(IRup=rownames(res_IC_IR_shCtrl_filter_058[res_IC_IR_shCtrl_filter_058$log2FoldChange>0,]),shDKC1down=rownames(res_IR_shDKC1_filter_058[res_IR_shDKC1_filter_058$log2FoldChange< 0,])),filename = "04_DEG_result/filter_0.58/IR_up_shDKC1_down.png",imagetype = "png",
             fill=c("#EECE13","#B210FF"),col=c("#EECE13","#B210FF"),width=2000,height = 2000,
               cat.just=list(c(3,0),c(-0.6,0)))

VennDiagram::venn.diagram(x=list(IRup=rownames(res_IC_IR_shCtrl_filter_058[res_IC_IR_shCtrl_filter_058$log2FoldChange<0,]),shDKC1down=rownames(res_IR_shDKC1_filter_058[res_IR_shDKC1_filter_058$log2FoldChange> 0,])),filename = "04_DEG_result/filter_0.58/IR_down_shDKC1_up.png",imagetype = "png",
                          fill=c("#EECE13","#B210FF"),col=c("#EECE13","#B210FF"),width=2000,height = 2000,
               cat.just=list(c(3,0),c(-0.6,0)))

VennDiagram::venn.diagram(x=list(IRup=rownames(res_IC_IR_shCtrl_filter_058[res_IC_IR_shCtrl_filter_058$log2FoldChange>0,]),shDKC1down=rownames(res_IR_shDKC1_filter_1[res_IR_shDKC1_filter_1$log2FoldChange< 0,])),filename = "04_DEG_result/filter_0.58/IR_up_shDKC1_down1.png",imagetype = "png",
                          fill=c("#EECE13","#B210FF"),col=c("#EECE13","#B210FF"),width=2000,height = 2000,
               cat.just=list(c(3,0),c(-0.6,0)))

VennDiagram::venn.diagram(x=list(IRup=rownames(res_IC_IR_shCtrl_filter_058[res_IC_IR_shCtrl_filter_058$log2FoldChange<0,]),shDKC1down=rownames(res_IR_shDKC1_filter_1[res_IR_shDKC1_filter_1$log2FoldChange> 0,])),filename = "04_DEG_result/filter_0.58/IR_down_shDKC1_up1.png",imagetype = "png",
                          fill=c("#EECE13","#B210FF"),col=c("#EECE13","#B210FF"),width=2000,height = 2000,
               cat.just=list(c(3,0),c(-0.6,0)))

venn.diagram(x=list(IRup=rownames(res_IC_IR_shCtrl_filter_1[res_IC_IR_shCtrl_filter_1$log2FoldChange>0,]),shDKC1down=rownames(res_IR_shDKC1_filter_058[res_IR_shDKC1_filter_058$log2FoldChange< 0,])),filename = "04_DEG_result/filter_0.58/IR_up1_shDKC1_down.png",imagetype = "png",
             fill=c("#EECE13","#B210FF"),col=c("#EECE13","#B210FF"),width=2000,height = 2000,
               cat.just=list(c(3,0),c(-0.6,0)))

VennDiagram::venn.diagram(x=list(IRup=rownames(res_IC_IR_shCtrl_filter_1[res_IC_IR_shCtrl_filter_1$log2FoldChange<0,]),shDKC1down=rownames(res_IR_shDKC1_filter_058[res_IR_shDKC1_filter_058$log2FoldChange> 0,])),filename = "04_DEG_result/filter_0.58/IR_down1_shDKC1_up.png",imagetype = "png",
                          fill=c("#EECE13","#B210FF"),col=c("#EECE13","#B210FF"),width=2000,height = 2000,
               cat.just=list(c(3,0),c(-0.6,0)))

bar <- data.frame(group=c("IR_up","IR_up","IR_down","IR_down"),shDKC1_reverse=c("yes","no","yes","no"),num=c(234,216,287,357))
...
pdf("04_DEG_result/filter_0.58/IR_DEG_reverse.pdf")
ggplot(bar)+
  geom_col(mapping = aes(x=group,y=num,fill=shDKC1_reverse),position = "stack")+
  scale_fill_brewer(palette = 'Set2')+
  geom_text(aes(x=group,y=num,label=num),position = position_stack(vjust = 0.5,reverse = F),size=7)+
  scale_y_continuous(limits = c(0, 287+357))+
  theme_bar
dev.off()
```
...
```{r GO filter 0.58}
dir.create("07_DEG_GO/filter0.58")

tt_enrichGO(genelist = rownames(res_IC_IR_shCtrl_filter_058)[res_IC_IR_shCtrl_filter_058$log2FoldChange>0],groupname = "IC_IR_shCtrl_filter058_up",OrgDb = org.Hs.eg.db,filedir = "07_DEG_GO/filter0.58",width = 8)
tt_enrichGO(genelist = rownames(res_IC_IR_shCtrl_filter_058)[res_IC_IR_shCtrl_filter_058$log2FoldChange<0],groupname = "IC_IR_shCtrl_filter058_down",OrgDb = org.Hs.eg.db,filedir = "07_DEG_GO/filter0.58",width = 8)

tt_enrichGO(genelist = rownames(res_IR_shDKC1_filter_058)[res_IR_shDKC1_filter_058$log2FoldChange>0],groupname = "IR_shDKC1_filter058_up",OrgDb = org.Hs.eg.db,filedir = "07_DEG_GO/filter0.58",width = 10)
tt_enrichGO(genelist = rownames(res_IR_shDKC1_filter_058)[res_IR_shDKC1_filter_058$log2FoldChange<0],groupname = "IR_shDKC1_filter058_down",OrgDb = org.Hs.eg.db,filedir = "07_DEG_GO/filter0.58",width = 8)


```

```{r GO reverse}
tt_enrichGO(genelist = read.table("04_DEG_result/filter_0.58/IR_down_shDKC1_up.txt")$V1,groupname = "IR_down_shDKC1_up",OrgDb = org.Hs.eg.db,filedir = "07_DEG_GO/filter0.58",width = 8)
tt_enrichGO(genelist = read.table("04_DEG_result/filter_0.58/IR_up_shDKC1_down.txt")$V1,groupname = "IR_up_shDKC1_down",OrgDb = org.Hs.eg.db,filedir = "07_DEG_GO/filter0.58",width = 8)
```
