...
cd ~/Data/09_PseudoU/06_aging_pU_SE/04_hisat2_mapping/05_polyT_merge_1
mkdir 01_sorted
...
# site排序
cd ~/Data/09_PseudoU/06_aging_pU_SE/04_hisat2_mapping/08_site_filter3/02_BS_count3_ratio3
  ...
  do 
    ...
    do 
      join -a 1 -1 1 -2 1 02_BS_count3_ratio3/site_name_all_sort.txt \
        ../05_polyT_merge_1/01_sorted/${i}_${j}_${k}_sorted.txt > 03_site_all_sample/site_all_${i}_${j}_${k}.txt & 
    done; 
  done; 
done
...
site_all$V2 <- sapply(strsplit(site_all$V1,split = "_",fixed = T),"[",1)
site_all$V3 <- sapply(strsplit(site_all$V1,split = "_",fixed = T),"[",2)
site_all$V4 <- sapply(strsplit(site_all$V1,split = "_",fixed = T),"[",3)

#site_all <- data.frame(chr=site_all$V2,pos=site_all$V3,strand=ifelse(site_all$V4=="T","+","-"))
site_all <- data.frame(chr=site_all$V2,pos=site_all$V3,strand=site_all$V4)
site_sen <- site_all
for (i in c("P12","P16")){
  for (k in c("1","2","3")){
    for (j in c("input","BS")){
...
cd ~/Data/09_PseudoU/06_aging_pU_SE/04_hisat2_mapping/08_site_filter3/
mkdir 05_site_motif
...
site_anno <- read.delim("06_site_anno/site_sen_anno_ensembl_basic_pc.txt",row.names = 1)
